...
public class FrontVersionController {
    ...
    public int deleteVersion(String histories){
        ...
        fileIdList.addAll(Arrays.asList(docIds));
  /*      boolean powerFlag = filesService.checkFileManageAuthority(fileIdList);
        if(!powerFlag){
            return 0;
        }*/
        return docVersionService.updateValidFlag(docIds);
    }
    ...
    public String setVersion(String oldDocId,String docId){
        ...
        idList.add(oldDocId);
 /*       boolean powerFlag = filesService.checkFileManageAuthority(idList);
        if(!powerFlag){
            return "false";
        }*/
        // 获取当前用户
       ...
       /* // 新增版本信息
        DocVersion oldVersion = docVersionService.selectOne(new EntityWrapper<DocVersion>().eq("doc_id", oldDocId));
        ...
        docVersionService.insert(newVersion);*/
        // 更新新版本应用时间
        ...
        boolean timeFlag = docVersionService.update(docVersion,new QueryWrapper<DocVersion>().eq("doc_id",docId));
        //更新置顶、广告位、专题文件
        ...
        return (infoFlag && esFlag && authorityFlag && validOld && validNew && timeFlag && TopFlag && BannerFlag) + "";
    }
}
