...
layui.use(['layer', 'table', 'HussarAjax', 'Hussar', 'form','laydate'], function() {
        ...
        var ajax = new $ax(Hussar.ctxPath + "/component/componentGraphCount", function(data) {
            ...
            var option = {
                ...
                tooltip: {
                    ...
                    formatter:'{b}<br />'+ '<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#5aa0f9;"></span>成果数量：'+'{c}'
                },
                grid: {
                    left: 50,
                    right: 0,
                ...
                yAxis: {
                    type: 'value',
                    max: function (value) {
                        return (Math.floor(value.max / 10) + 1) * 10 + 10;
                    },
                    axisLine: {
                        show: false
                    },
        ...
        var ajax = new $ax(Hussar.ctxPath + "/multiplex/multiplexGraphCount?", function(data) {
            ...
            var option = {
                ...
                tooltip: {
                    ...
                    formatter:'{b}<br />'+ '<span style="display:inline-block;margin-right:5px;border-radius:10px;width:10px;height:10px;background-color:#5aa0f9;"></span>复用数量：'+'{c}'
                },
                grid: {
                    left: 50,
                    right: 0
    ...
    Statistics.initClickEvent = function () {
        $(".switch>div").click(function () {
            ...
            //获取当前点击的时间范围值
            Statistics.dateNum = $(this).attr("value");
    ...
    Statistics.initClickReuse = function () {
        $(".date>div").click(function () {
            ...
            $(this).addClass("active");
            //获取当前点击的时间范围值
            Statistics.dateNum2 = $(this).attr("value");
        ...
        $(".bg>div").click(function () {
            ...
            $(this).addClass("active");
            //获取当前点击的时间范围值
            Statistics.syb2 = $(this).attr("value");
            Statistics.flushReuse();
        });
    ...
    Statistics.refreshEchart = function () {
        var ajax = new $ax(Hussar.ctxPath + "/component/componentGraphCount?date="+Statistics.dateNum+"&state="+Statistics.state+"&bu="+Statistics.syb+"&order="+Statistics.dept, function(data) {
            var  firEchart = echarts.init(document.getElementById('firchart'));
            var option = {
                backgroundColor: '#fff',
                tooltip: {
    ...
    Statistics.flushReuse = function () {
        var ajax = new $ax(Hussar.ctxPath + "/multiplex/multiplexGraphCount?date="+Statistics.dateNum2+"&bu="+Statistics.syb2+"&order="+ Statistics.dept2, function(data) {
            var  firEchart = echarts.init(document.getElementById('reuse'));
            var option = {
                backgroundColor: '#fff',
                tooltip: {
