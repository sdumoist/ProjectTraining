...
layui.use(['jquery',  'tree', 'table', 'util', 'laytpl', 'form', 'layer','Hussar','HussarAjax'], function () {
    ...
    TopicBin.initTableView = function () {
        tableIns =table.render({
            ...
            , url: Hussar.ctxPath+'/exempt/exemptList' //数据接口
            , request: {
            ...
            ,even:true
            , cols: [[
                {field: 'fileId', title: 'id', type: 'checkbox', width: '5%', align: "center"},
                {type: 'numbers', title: '序号', align: 'center', width: '5%'},
                {field: 'docName', title: '文件名', width: '40%', align: "left",},
                {field: 'fileType', title: '是否为文件夹', width: '15%', align: "center",templet: '#type'},
                {field: 'createTime', title: '创建时间', width: '35%', align: "center"},
            ]] //设置表头
        });
        //.监听选择，记录已选择项
        table.on('checkbox(topicList)', function (obj) {
            ...
            $.each(data, function (k, v) {
                ...
                if (obj.checked) {
                    //.增加已选中项
                    layui.data('checked', {
                        key: v.fileId, value: v
                    });
                } else {
                    //.删除
                    layui.data('checked', {
                        key: v.fileId, remove: true
                    });
                }
            });
        });
    ...
    active = {
       ...
       , moveUp: function () {
            ...
            if ($tr.index() != 0) {
                /*$.ajax({                        //交换两行的showorder
                    ...
                    data:{
                        table:"doc_top_file",
                        idColumn:"top_id",
                        idOne:dataArr[0].fileId,
                        idTwo:data[upNum].fileId
                    },
                    async:false,
                    cache:false,
                    dataType:"json",
                ...
                ajax.set("idOne",dataArr[0].fileId);
                ajax.set("idTwo",data[upNum].fileId);
                ajax.start();
                /*$tr.fadeOut().fadeIn();
                 $tr.prev().before($tr);*/
            }else{
            ...
            if(next){
                ...
                ajax.set("idColumn","top_id");
                ajax.set("idOne",dataArr[0].fileId);
                ajax.set("idTwo",data[downNum].fileId);
                ajax.start();
                /*$tr.fadeOut().fadeIn();
                 $tr.next().after($tr);*/
            }
        ...
        , delTopics: function () {
            ...
            for (var i = 0; i < dataArr.length; i++) {
                layui.data('checked', {
                    key: dataArr[i].fileId, remove: true
                });
                if (i == 0) {
                    ids = dataArr[i].fileId;
                } else {
                    ids += ',' + dataArr[i].fileId;
                }
            }
            layer.confirm('确定要取消所选中的文件积分免除吗？', function () {
                /*$.ajax({
                    type: "post",
                    url: Hussar.ctxPath+"/exempt/delExempt",
                    data: {
                        ids: ids
                    },
                    async: false,
                ...
                var ajax = new $ax(Hussar.ctxPath + "/exempt/delExempt", function(data) {
                    if (data == dataArr.length) {
                        layer.alert('取消成功', {
                            icon: 1,
                            shadeClose: true,
