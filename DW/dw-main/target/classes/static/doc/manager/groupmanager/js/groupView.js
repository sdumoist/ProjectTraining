...
layui.use(['jquery', 'layer', 'Hussar', 'jstree', 'table', 'HussarAjax', 'form', 'element','Hussar'], function () {
    ...
    RoleManage.initButtonEvent = function () {
        ...
        $("#addBtn").on('click', function () {
            ...
            if (isSort == "0") {
                ...
                openTopic('新增群组', '/group/groupAdd?sortId=' + $("#groupId").val(), 800, 550);
            }
        });
        ...
        $("#upBtn").on('click',function(){
            ...
            if ($("#loginId").val() == 'admin' ||  $('#loginId').val() == $('#usersId').val()) {
                if($("#isSort").val() == '0'){
                    ...
                    openEdit('修改群组', '/group/groupEdit', id, 800, 550);
                }else{
                    buttonType = 'editGroup';
                    //openEdit('分组修改', '/group/sortEdit', id, 620, 160);
                    openSort('修改分组',id)
                }
            } else {
        ...
        $("#addSortBtn").on('click', function () {
            ...
            if (isSort == "0") {
                ...
                //openTopic('新增分组', '/group/sortAdd?groupId=' + $("#groupId").val() + '&groupName=' + $("#groupName").val(), 620, 160);
                openSort('新增分组','');
            }
        });
    }
                ...
                'action': function () {
                    if ($("#loginId").val() == 'admin' || $("#loginId").val() == node.original.user) {
                        if(node.original.issort == '0'){
                            buttonType = 'editGroup';
                            openEdit('修改群组', '/group/groupEdit', node.id, 800, 550);
                        }else{
                            buttonType = 'editGroup';
                            //openEdit('分组修改', '/group/sortEdit', node.id, 620, 160);
    ...
    function openEdit(title, url, id, w, h) {
        layui.use(['Hussar'], function(){
            ...
            layer.open({
                ...
                shade: 0.4,
                title: title,
                content: Hussar.ctxPath+url + "?groupId=" + id
            });
        });
    };
    //分组新增
    function openSort(name,id) {
        layui.use(['Hussar'], function(){
            ...
            if(buttonType == 'editGroup'){
                /*$.ajax({
                    type: "post",
                    url: Hussar.ctxPath+'/group/sortEdit',
                    data: {
                        groupId: id,
                    },
                    async: true,
    ...
    function delGroupUser() {
        layui.use(['Hussar'], function(){
            ...
            layer.confirm('确定要删除所选中的群组人员吗？', function () {
                /*$.ajax({
                    type: "post",
