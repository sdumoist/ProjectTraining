...
layui.use([ 'layer', 'jquery', 'form', 'table', 'bootstrap_table_edit', 'Hussar', 'HussarAjax' ], function() {
	var $ = layui.jquery, form = layui.form, layer = layui.layer, table = layui.table, Hussar = layui.Hussar, $ax = layui.HussarAjax;

	/**
	 * 初始化表格的列
	 */
	Statistics.initEchar = function(url) {
		// getDeptData getUserPreviewData
	...
	$(function() {
		...
		var url = "/fileStatistics/getDeptData";
		form.on('select(opType)', function(data) {
			if (data.value == "dept") {
				url = "/fileStatistics/getDeptData";
				...
				if (data.value == "user") {
					url = "/fileStatistics/getUserPreviewData";
					$("#content").css("display", "block");
					$("#main").css("display", "block");
					Statistics.initEchar(url);
					Statistics.myChart.resize();
					console.log(Statistics.tableIns);
					//隐藏table
					$(".layui-table-box").parent().css("display", "none");
					$("#fileDataLogTable").css("display", "none");
				}
