    ...
    var gridView ={
        ...
        init:function(){
            ...
            layui.use(['Hussar','HussarAjax'], function(){
                ...
                /*$.ajax({
                    async:false,
                    type:"post",
                    url: Hussar.ctxPath+"/sharefile/fileDetail",
                    data:{hash:hash},
                    success:function(data) {
                        ...
                        document.getElementById("showImg").src = Hussar.ctxPath+"/preview/listForShare?hash="+hash;
        ...
        downloadFile:function(){//文档ID，作者ID，下载消耗积分,文档名，分类,文件路径
            ...
            layui.use(['Hussar','HussarAjax'], function(){
                ...
                /*$.ajax({
                    type:"post",
                    url: Hussar.ctxPath+"/file/changePoints",
                    async:false,
                    cache:false,
                    data:{
                        authorId:authorId,
...
function loginSubmit(){
    ...
    layui.use(['Hussar','HussarAjax'], function(){
        ...
        /*$.ajax({
            url: Hussar.ctxPath+"/index/loginCheck",
            type: "post",
            async: "false",
            dataType: "text",
...
function showDoc(fileType,id) {
    ...
    layui.use(['Hussar','HussarAjax'], function(){
        ...
        if(fileType=="png"||fileType=="jpg"||fileType=="gif"||fileType=="bmp"||fileType=="jpeg"){
            openWin(Hussar.ctxPath+"/preview/toShowIMG?id=" + id + "&fileType=" + selectVal + "&keyWords=" + encodeURI(keyWords), "_self");
        }else if(fileType=="mp4"||fileType=="wmv"){
            openWin(Hussar.ctxPath+"/preview/toShowVideo?id=" + id + "&fileType=" + selectVal + "&keyWords=" + encodeURI(keyWords), "_self");
        } else if(fileType=="mp3"||fileType=="m4a"){
            openWin(Hussar.ctxPath+"/preview/toShowVoice?id=" + id + "&fileType=" + selectVal + "&keyWords=" + encodeURI(keyWords), "_self");
            ...
            openWin(Hussar.ctxPath+"/preview/toShowPDF?id=" + id + "&fileType=" + selectVal + "&keyWords=" + encodeURI(keyWords), "_self");
        }else {
            openWin(Hussar.ctxPath+"/preview/toShowOthers?id=" + id + "&fileType=" + selectVal + "&keyWords=" + encodeURI(keyWords), "_self");
        }
    });
}
