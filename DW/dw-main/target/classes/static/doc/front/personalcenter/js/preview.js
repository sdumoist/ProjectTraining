...
layui.use(['form', 'laypage', 'jquery','layer','laytpl','Hussar'], function() {
            ...
            success: function (data) {
                if (data.status == "1") {
                    var index2 = layer.confirm('下载文件将扣除'+data.integral+'积分，是否确认下载？', {icon: 3, title: '提示'}, function (index) {
                        ...
                        $.ajax({
                            url: Hussar.ctxPath+"/integral/integralForMulDownLoad",
                            async: true,
                            data: {
                                ids: ids,
                                ruleCode: 'download'
    ...
    $("#clearHistoryBtn").on('click',function(){
        ...
        layer.confirm('确定要清空浏览记录吗？',{title :['清空浏览记录','background-color:#fff'],offset:scrollHeightAlert,skin:'move-confirm'},function(){
            var index = layer.load(1, {
                shade: [0.1,'#fff'], //0.1透明度的白色背景
                offset: scrollHeightAlert
            });
