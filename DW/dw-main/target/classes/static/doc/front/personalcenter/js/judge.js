...
layui.use(['form', 'laypage', 'jquery', 'layer', 'laytpl', 'Hussar'], function () {
    ...
    hussar = layui.Hussar;
    imgs = {
        folderMember1: Hussar.ctxPath + "/static/resources/img/fsfile/folder_member1.png",
        folderMember2: Hussar.ctxPath + "/static/resources/img/fsfile/folder_member2.png",
        word: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/word.png",
        txt: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/txt.png",
        ppt: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/ppt.png",
        pdf: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/pdf.png",
        ceb: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/ceb.png",
        music: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/music.png",
        video: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/video.png",
        excel: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/excel.png",
        pic: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/pic.png",
        bmp: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/bmp.png",
        psd: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/psd.png",
        html: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/html.png",
        exe: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/exe.png",
        rar: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/rar.png",
        other: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/other.png",
        entry: Hussar.ctxPath + "/static/resources/img/front/file-iconNew/entry.png",
    };
    //初始化树
    start();
 ...
 * @param name
 */
function pass(e, id, name) {
    layui.use(['layer', 'Hussar', 'HussarAjax'], function () {
        var layer = layui.layer;
        var Hussar = layui.Hussar,
            $ax = layui.HussarAjax;
        layer.confirm('确定要通过吗？', {
            title: ['通过', 'background-color:#fff'],
            offset: scrollHeightAlert,
            skin: 'move-confirm'
        }, function () {
            var ajax = new $ax(Hussar.ctxPath + "/docInfoAudit/approved", function (data) {
                Hussar.success('通过成功');
                refreshFile(null, null, null);
            }, function (data) {
    ...
    layui.use(['layer', 'Hussar', 'HussarAjax'], function () {
        ...
        var operation = function () {
            ...
            layer.open({
                ...
                shadeClose: false,
                shade: 0.4,
                title: ["驳回", 'background-color:#fff'],
                content: $('#rejectDiv'),
                btn1: function (index, layero) {
                    var opinion = $("#auditOpinion").val();
                    if ("" == opinion) {
                        layer.msg("驳回原因不能为空", {anim: 6, icon: 0, offset: scrollHeightMsg});
                        return;
                    }
                    var ajax = new $ax(Hussar.ctxPath + "/docInfoAudit/reject", function (data) {
                        layer.closeAll();
                        Hussar.success('驳回成功');
                        refreshFile(null, null, null);
                    }, function (data) {
                    ...
                    ajax.set("id", id);
                    ajax.set("opinion", opinion);
                    ajax.start();
                }
            });

        };
        index = layer.confirm("确定要驳回么？", {
            title: ['驳回', 'background-color:#fff'],
            offset: scrollHeightAlert,
            skin: 'move-confirm'
        }, operation);
    ...
    if (checks()) {
        var operation = function () {
            layui.use(['layer', 'Hussar', 'HussarAjax'], function () {
                ...
                ajax.set("id", chooseFile.join(","));
                ajax.start();
            });
        };
        layer.confirm("确定要通过么？", {
            title: ['通过', 'background-color:#fff'],
            offset: scrollHeightAlert,
            skin: 'move-confirm'
        }, operation);
...
function refreshFile(num, size, order) {
    ...
    layui.use(['laypage', 'layer', 'table', 'Hussar', 'HussarAjax'], function () {
        ...
        var ajax = new $ax(Hussar.ctxPath + "/docInfoAudit/myApprovedList", function (data) {
            laypage.render({
                elem: 'laypageAre'
    ...
    layui.use(['Hussar', 'HussarAjax'], function () {
        ...
        var ajax = new $ax(Hussar.ctxPath + "/personalcenter/getInfo", function (data) {
            if (data.result == "1") {
                ...
                layer.msg("该文件待审批", {anim: 6, icon: 0});
            } else if (data.result == "7") {
                layer.msg("该文件被驳回", {anim: 6, icon: 0});
            } else if (data.result == "5") {
                layer.msg("该文件不是最新版本", {anim: 6, icon: 0});
            } else {
                var title = '';
...
function iconMove(e, id, type, name, author) {
    ...
    var index = layer.confirm('确定要移动所选文件吗？', {
        title: ['移动', 'background-color:#fff'],
        offset: scrollHeightAlert,
        skin: 'move-confirm'
    }, operation);
...
function showPdf(id, fileSuffixName, name) {
    ...
    layui.use(['Hussar', 'HussarAjax'], function () {
        ...
        /*$.ajax({
            ...
            success: function (data) {
                if(data.code==1){
                    window.open(Hussar.ctxPath+"/preview/toShowPDF?id=" + id);
                }else if(data.code==2){
                    window.open(Hussar.ctxPath+"/preview/toShowIMG?id=" + id);
                }else if(data.code==3){
                    window.open(Hussar.ctxPath+"/preview/toShowVideo?id=" + id);
                }else if(data.code==4){
                    window.open(Hussar.ctxPath+"/preview/toShowVoice?id=" + id);
                }else{
                    window.open(Hussar.ctxPath+"/preview/toShowOthers?id=" + id);
                }
...
$(function () {
    ...
    setInterval(function () {
        ...
        if (scrollHeight != 0) {
            ...
            scrollHeightAlert = parseInt((screenHeight - 154) / 2.0 - 130) + "px";
            scrollHeightLong = parseInt((screenHeight - 510) / 2.0 - 30) + "px";
            scrollHeightTip = parseInt((screenHeight - 250) / 2.0 - 130) + "px";
            scrollHeightShare = parseInt((screenHeight - 200) / 2.0 - 130) + "px";
            scrollHeightMsg = parseInt((screenHeight - 64) / 2.0 - 130) + "px";
        }
   }, 300);
