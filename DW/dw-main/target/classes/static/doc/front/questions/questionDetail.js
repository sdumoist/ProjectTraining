...
layui.use(['layedit','layer','Hussar','HussarAjax','form'], function() {
    ...
    $("#endProblem").on('click',function () {
        ...
        layer.confirm("确定要结束问题吗？",{skin: 'layerpop-title-class1'}, operation);
    })
    ...
    if(commentObj.length > 0){
        ...
        if(res != null && res.length >0){ // 有子评论
            ...
            html = '<div class="comment-area">\n' +
                ...
                '                                <pre></pre><textarea placeholder="快来留下你的评论吧～" onfocus="textareaFoucs(this)" onblur="textareaBlur(this)" oninput="wordLeg(this);" maxlength="200" onpropertychange="if(value.length>200) value=value.substr(0,200)" name="desc" class="layui-textarea"></textarea>\n' +
                '                                <div class="word"><span class="text_count">0</span>&nbsp;/&nbsp;<span class="num_count">200</span></div>\n' +
                '                            </div>\n' +
                '                            <button class="submit-comment-btn" queId="'+queId+'" ansId="'+ansId+'" byReplyId="'+byReplyId+'" onclick="published(this)" disabled style="display: none">发表</button>\n' +
                '                        </div>\n' +
...
function replyComment(ele) {
    ...
    if(objItem.length>0){
        ...
        var html = '<div class="comment-input-container comment-input-container1">\n' +
            '                                    <div class="textarea-container">\n' +
            '                                        <pre></pre><textarea placeholder="请输入" onfocus="textareaFoucs1(this)" onblur="textareaBlur(this)" oninput="wordLeg(this);" maxlength="200" onpropertychange="if(value.length>200) value=value.substr(0,200)" name="desc" class="layui-textarea"></textarea>\n' +
            '                                        <div class="word"><span class="text_count">0</span>&nbsp;/&nbsp;<span class="num_count">200</span></div>\n' +
            '                                    </div>\n' +
            '                                   <button class="submit-comment-btn" queId="'+queId+'" ansId="'+ansId+'" byReplyId="'+byReplyId+'" onclick="published2(this)" disabled style="display: none">发表</button>\n' +
            '                                </div>';
