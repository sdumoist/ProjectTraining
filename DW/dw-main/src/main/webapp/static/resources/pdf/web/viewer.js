...
/******/([
    ...
    /***/ (function (module, exports, __webpack_require__) {
        ...
        var PDFViewerApplication = {
            ...
            error: function error(message, moreInfo) {
                ...
                var obj_parent=window.parent.document.getElementById('openPage');
                obj_parent.style.display = "none";
                var obj_parent_1=window.parent.document.getElementById('open');
                obj_parent_1.style.display = "none";
                var obj= document.getElementById('mainContainer');
                obj.style.cssText = "border-bottom:0;";
                var obj_footer = window.parent.document.getElementById('error-footer');
        ...
        function webViewerPresentationMode() {
            if(parent.docType=="doc"||parent.docType=="docx"){
            ...
            _createClass(PDFFindBar, [{
                ...
                value: function updateResultsCount(matchCount) {
                    ...
                    if (!matchCount) {
                        ...
                        this.findResultsCount.textContent = matchCount;
            ...
            _createClass(PDFPageView, [{
                ...
                value: function draw() {
                    ...
                    if (this.textLayerFactory) {
                        ...
                        var topPotision = "";
                        var cover_status = 0;
                        if (canvasWrapper.width > canvasWrapper.height) { //pdf宽度大于长度时，create 遮罩 减半
                            loopNum = 2
                            leftPotision = "-10%";
                            topPotision = "50%";
                        }
                        //取传递参数 file  username
                        var reg1 = new RegExp("(^|&)" + 'username' + "=([^&]*)(&|$)", i); // 匹配目标参数（当前用户名）
                        var result2 = window.location.search.substr(1).match(reg1); // 对querystring匹配目标参数
                        var markUserName = decodeURIComponent(result2[2]);
                        var reg2 = new RegExp("(^|&)" + 'watermark_user_flag' + "=([^&]*)(&|$)", i); // 匹配目标参数（是否添加用户水印）
                        var result3 = window.location.search.substr(1).match(reg2); // 对querystring匹配目标参数
                        cover_status = decodeURIComponent(result3[2]);
                        var reg3 = new RegExp("(^|&)" + 'watermark_company_flag' + "=([^&]*)(&|$)", i); // 匹配目标参数（当前用户名）
                        var result4 = window.location.search.substr(1).match(reg3); // 对querystring匹配目标参数
                        var watermarkCompanyFlag = decodeURIComponent(result4[2]);
                        var reg4 = new RegExp("(^|&)" + 'companyValue' + "=([^&]*)(&|$)", i); // 匹配目标参数（当前用户名）
                        var result5 = window.location.search.substr(1).match(reg4); // 对querystring匹配目标参数
                        var companyValue = decodeURIComponent(result5[2]);
                        var watermark = '';
                        if (cover_status == 1 && watermarkCompanyFlag == 1) {
                            watermark = "\r\n " + markUserName + "\r\n" + companyValue;
                        }else if(cover_status == 1 && watermarkCompanyFlag == 0){
                            watermark = "\r\n " + markUserName;
                        }else if(cover_status == 0 && watermarkCompanyFlag == 1){
                            watermark = "\r\n" + companyValue;
                        }else{
                            watermark = '';
                        }
                        for (var kk = 0; kk < loopNum; kk++) {
                            if (kk % 2 == 0) {
                                continue;
                            }
                            var cover = document.createElement('div');
                            cover.className = "cover" + kk;
                            cover.innerText = watermark;
                            ...
                            var cover = document.getElementsByClassName('cover' + kk);
                            var size = 0;
                            var nowWidth = canvasWrapper.style.width.split("p")[0];
                            //714为100%时，每页的宽度。对比当前的宽度可以计算出页面变化后字体的数值
                            var size = 20 * nowWidth / 714 + "px";
                            for (var i = 0, len = cover.length; i < len; i++) {
                                ...
                                if (leftPotision != "") {
                                    cover[i].style.left = leftPotision;
                                    cover[i].style.top = topPotision;
                                }
                            }
                        }
                        if (this.annotationLayer && this.annotationLayer.div) {
                ...
                value: function _adjustScaleWidth() {
                    ...
                    _ui_utils.animationStarted.then(function () {
                        ...
                        if (container.clientWidth > 0) {
                            ...
                            select.setAttribute('style', 'min-width: ' + (width) + 'px;');
    ...
    /***/ (function (module, exports, __webpack_require__) {
        ...
        function webViewerLoad() {
            ...
            //HTML文档被加载和解析完成后触发
            var obj = document.getElementById("viewerContainer");
            obj.scrollTop = 0;
            window.parent.iframeScroll()
        }
