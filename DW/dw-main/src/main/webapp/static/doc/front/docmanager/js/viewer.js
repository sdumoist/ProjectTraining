...
(function (global, factory) {
  ...
  var TEMPLATE = '<div class="viewer-container" touch-action="none">' + '<div class="viewer-canvas" oncontextmenu = "return false;"></div>' + '<div class="viewer-footer">' + '<div class="viewer-title"></div>' + '<div class="viewer-toolbar"></div>' + '<div class="viewer-navbar">' + '<ul class="viewer-list"></ul>' + '</div>' + '</div>' + '<div class="viewer-tooltip"></div>' + '<div role="button" class="viewer-button" data-viewer-action="mix"></div>' + '<div class="viewer-player"></div>' + '</div>';
  ...
  var render = {
    ...
    renderList: function renderList(index) {
      ...
      var width = this.items[i].offsetWidth || 100 + 8;
      var outerWidth = width + 9; // 1 pixel of `margin-left` width
      // Place the active item in the center of the scree
      var thisW = 8;
      for(var a =0 ;a<this.length;a++){
        if(!this.items[a]){
          continue;
        }
          console.log(this.items[a].offsetWidth || 100);
          thisW = thisW + 11 + (this.items[a].offsetWidth || 100);
      }
      console.log(thisW);
  ...
  var handlers = {
    ...
    loadImage: function loadImage(e) {
      ...
      getImageNaturalSizes(image, function (naturalWidth, naturalHeight) {
        ...
        if(document.getElementsByClassName('viewer-canvas')[0].getElementsByClassName("watermark").length==0){
          var shuiyinadmin =document.getElementById('watermark_user_flag').value==1?document.getElementById('username').value:''
          var shuiyincomy =document.getElementById('watermark_company_flag').value==1?document.getElementById('companyValue').value:''
          var inner = `<div class="watermark"><div>${shuiyinadmin}</div><div>${shuiyincomy}</div></div>`
          var cover6 = document.createElement('div')
          cover6.innerHTML=inner
          document.getElementsByClassName('viewer-canvas')[0].appendChild(cover6)
        }
      });
    },
