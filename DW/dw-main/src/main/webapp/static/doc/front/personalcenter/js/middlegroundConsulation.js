...
layui.use(['form', 'laypage', 'jquery', 'layer', 'laytpl', 'Hussar','element'], function () {
    ...
    $("#componentApply").on('click', function () {
        ...
        location.href = "/middlegroundConsulation/toAddMeetingReord?type="+typeStr+"&origin="+originStr+"&state="+stateStr;
    });
    $("#componentApplyAll").on('click', function () {
        ...
        location.href = "/middlegroundConsulation/toAddMeetingReord?type="+typeStr+"&origin="+originStr+"&state="+stateStr+"&dept="+deptStr;
    });
    //修改
    $("#updateComponentApplyAll").on('click', function () {
        ...
        if (chooseFile.length > 1) {
            ...
            location.href = "/middlegroundConsulation/toEditMeetingReord?componentId=" + chooseFile[0]+"&type="+typeStr+"&origin="+originStr+"&state="+stateStr
        }
    ...
    $("#updateComponentApply").on('click', function () {
        ...
        if (chooseFile.length > 1) {
            ...
            location.href = "/middlegroundConsulation/toEditMeetingReord?consulationId=" + chooseFile[0]
        }
    ...
    $("#updateMultiplex").on('click', function () {
        ...
        if (chooseFile.length > 1) {
            ...
            location.href = "/multiplex/multiplexUpdateView?projectId=" + chooseFile[0];
        }
    ...
    $("#multiplexList").on('click', function () {
        layer.open({
            ...
            shade: 0.4,
            title: "我的复用",
            content: "/multiplex/myMultiplexView"
        });
    });
    $("#multiplexApply").on('click', function () {
        location.href = "/multiplex/multiplexApply"
    });
    ...
    $("#delComponentBtn").on('click', function () {
        ...
        layer.confirm('确定要删除所选记录吗？', {title: ['删除', 'background-color:#fff'], offset: scrollHeightAlert,skin:'move-confirm'}, function () {
            var index = layer.load(1, {
                shade: [0.1, '#fff'] //0.1透明度的白色背景
            });
            if (chooseFile.length == 0) {
            ...
            $.ajax({
                type: "post",
                url: "/middlegroundConsulation/deleteMiddlegroundConsulation",
                contentType: "application/x-www-form-urlencoded;charset=UTF-8",
                data: {
                    middleGroundConsulationId: scopeId
                },
                async: true,
                cache: false,
                success: function (data) {
    ...
    //     }
    //
    //     layer.confirm('确定要删除所选复用登记吗？', {title: ['删除', 'background-color:#fff'], offset: scrollHeightAlert}, function () {
    //         var index = layer.load(1, {
    //             shade: [0.1, '#fff'] //0.1透明度的白色背景
    //         });
    //         if (chooseFile.length == 0) {
    ...
    //         $.ajax({
    //             type: "post",
    //             url: "/multiplex/deleteMultiplexSave",
    //             data: {
...
function refreshFile(num, size, order) {
    ...
    currOrder = order;
    if(style==1){
        $("#searchName").attr("placeholder","按名称查找记录");
        var type=[];
        var state=[];
        var origin=[];
        $('input[name="type"]:checked').each(function(){//遍历每一个名字为state的复选框，其中选中的执行函数
        ...
        $("#marg .file-container-flatten").css("margin-top","120px")
        ...
        layui.use(['laypage', 'layer', 'table'], function () {
            ...
            $.ajax({
                type: "post",
                url: "/middlegroundConsulation/getMiddlegroundConsulationList",
                contentType: "application/x-www-form-urlencoded;charset=UTF-8",
                data: {
                    isMy:true,
                    page: num,
        ...
        $(".opens").click(function () {
            ...
            $(".closes").show()
            $("#marg .file-container-flatten").css("margin-top","264px");
...
function dbclick(id, type, name,componentState) {
    openWin("/toShowConsulation/toShowPDF?id=" + id);
...
function projectCheck(userName,componentName,style){
    layer.open({
        ...
        content: "/component/componentPersonage?userName=" + userName+"&style"+style
    });
}
function viewReason(componentId){
    layer.open({
        ...
        shade: 0.4,
        title: "驳回意见",
        content: "/multiplex/viewReason?componentId=" + componentId
    });
}
