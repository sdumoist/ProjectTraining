...
layui.use(['form', 'jquery','util','layer','table','Hussar','jstree','HussarAjax','element','laydate','upload'], function() {
        ...
        if (obj.event == 'delete') {
            ...
            if (docTopicId == null || docTopicId == "" || docTopicId == undefined){
                ...
                layer.confirm('确定要删除所选中的文件吗？',function(){
                    ...
                    var ajax = new $ax(Hussar.ctxPath + "/topic/delDoc", function(data) {
                        if(data == dataArr.length){
                            layer.alert('删除成功', {
                                icon :  1,
                                shadeClose: true,
    ...
    $("#saveBtn").on('click',function(){
        var showOrder = $("#showOrder").val().trim();//专题名称
        ...
        if(showOrder == ""){
            layer.msg("排序不能为空", {anim:6,icon: 0});
            return;
        }
        if(topicName == ""){
    ...
    var active = {
        ...
        ,del:function(){
            ...
            var ajax = new $ax(Hussar.ctxPath + "/topic/delDocById", function(data) {
                if(data != dataArr.length){
                    layer.alert('删除失败', {
                        icon :  0,
                        shadeClose: true,
