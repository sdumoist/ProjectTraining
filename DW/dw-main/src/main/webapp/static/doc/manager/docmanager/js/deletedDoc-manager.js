...
layui.use(['form', 'laypage', 'jquery','layer','Hussar','jstree','laytpl'], function() {
    ...
    $("#delBtn").on('click',function(){
        ...
        layer.confirm('确定要删除所选记录吗？',function(){
            ...
            var ajax = new $ax(Hussar.ctxPath + "/fsFile/delDoc", function(data) {
                if(data> 0){
                    Hussar.success('删除成功')
                }else {
                    Hussar.error('删除异常')
...
function refreshFile(num,size){
    layui.use(['laypage','layer','table','Hussar','HussarAjax'], function(){
        ...
        var ajax = new $ax(Hussar.ctxPath + "/fsFile/getDeletedFileList", function(data) {
            laypage.render({
                elem: 'laypageAre'
                ,count: data.total //数据总数，从服务端得到
                ,limit: window.limit = 20
