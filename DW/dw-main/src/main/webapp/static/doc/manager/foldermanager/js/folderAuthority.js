...
layui.use(['layer','Hussar', 'HussarAjax','form','table','jquery','element','jstree'], function(){
    ...
    $(function () {
        ...
        $("#saveBtn").on('click',function(){
            ...
            var categoryName = parent.chooseFileName.join(",");
            var folderId = parent.chooseFile.join(",");
            var visible = '';
            ...
            // var operation = function(){
                /*$.ajax({
                    type: "POST",
                    url: Hussar.ctxPath+"/fsFolder/editAuthority",
                    data : {
                    ...
                    success:function(result) {
                        if(result){
                            ...
                            parent.layer.msg("授权失败", {icon: 0,offset:parent.scrollHeightMsg});
                        }
                    }
                });*/
            var ajax = new $ax(Hussar.ctxPath + "/fsFolder/editAuthority", function(result) {
                if(result){
                    parent.layer.msg("授权成功", {icon: 1,offset:parent.scrollHeightMsg});
                    var index = parent.layer.getFrameIndex(window.name);
                    parent.layer.close(index);
                    ...
                    parent.layer.msg("授权失败", {icon: 0,offset: 'b'});
                }
            }, function(data) {
            ...
            ajax.set("succeedFlag", succeedFlag);
            ajax.set("authType",$("#authType").val());
            ajax.start();
            // }
            // layer.confirm('确定要重新授权？',{title:['授权','background-color:#fff']}, operation);

        });
        /*权限说明  鼠标滑过事件*/

        // 下拉框悬停样式
        //$(".layui-form-select .layui-input").hover(function () {
        //    $(this).css("border","1px solid rgba(178,194,213,1)");
        //    $(this).parent().find(".layui-edge").css("opacity","1");
        //}, function () {
        //    $(this).css("border","1px solid #fff");
        //    $(this).parent().find(".layui-edge").css("opacity","0");
        //});
        /*群组，人员tab的切换监听*/
                ...
                if(checkParent){
                    for(var i = 0;i<data.length;i++){
                        ...
                        if(data[i].authorType == "0"){
                            ...
                            var datajson={"id":data[i].authorId,"name":name,"type":data[i].authorType,"organId":data[i].organId,"operateType":data[i].operateType};
    ...
    function initRitTable() {
        ...
        if((roleInit!=undefined&&roleInit.length>0)||(personInit!=undefined&&personInit.length>0)||(groupInit!=undefined&&groupInit.length>0)){
            initChooseTable([]);
            table.reload('rit_table',{
                data : roleInit.concat(personInit.concat(groupInit))
            });
            choose = roleInit.concat(personInit.concat(groupInit));
        }else {
            initChooseTable([]);
        }
    }
    ...
    function checkParent() {
        if (parent.chooseFile.length == 1) {
            var folderId = parent.chooseFile[0];
            var ajax = new $ax(Hussar.ctxPath + "/fsFolder/detail", function (data) {
                if(data != null && data.succeedFlag == '1'){
