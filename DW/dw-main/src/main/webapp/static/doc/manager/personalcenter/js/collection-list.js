...
layui.use(['form', 'laypage', 'jquery','layer','Hussar','jstree','laytpl'], function() {
    ...
    $("#delHistoryBtn").on('click',function(){
        ...
        if(chooseFile.length==0){
            layer.msg("请选择要取消收藏的记录", {anim:6,icon: 0});
            return;
        }

        layer.confirm('确定要取消收藏吗？',function(){
            var index = layer.load(1, {
                shade: [0.1,'#fff'] //0.1透明度的白色背景
            });
            if(chooseFile.length==0){
            ...
            $.ajax({
                type:"post",
                url:"/personalCollection/deleteCollection",
                data:{
                    ids: histories,
                    opType: opType
                },
    ...
    $("#clearHistoryBtn").on('click',function(){
        ...
        layer.confirm('确定要清空下载记录吗？',function(){
            ...
            $.ajax({
                type:"post",
                url:"/personalCollection/clearHistory",
                data: {opType: opType},
                async:true,
                cache:false,
                success:function(data){
